...
const EditTaskModal = ({
  ...
  //
  useEffect(() => {
    const task = taskData.tasks.find((data) => data._id === taskid);
    setTitle(task.subtitle);
    setDescription(task.subdescription);
    setAssignedBy(task.subassignedBy);
    setPriority(task.priority);
    setDateField(new Date(task.dueDate));
    const initialAssignedMembers = task.subassignedTo.map((userId) => {
      const user = UsersList.find((u) => u._id === userId.toString());
      ...
      try {
        await axios
          ...
          .then(() => {
            ...
            toast.success("Task Updated Succcessfully");
            onClose();
          });
      } catch (e) {
      ...
      <div className='bg-white w-full max-w-md p-6  rounded-md shadow-md'>
        ...
        <form>
          ...
          <div className='mb-4'>
            ...
            <div className='relative'>
              <input
                ...
                value={assignedBy}
                disabled={true}
                className='w-full border rounded-md py-2 px-3 pl-10'
              />
          ...
          <div className='mb-4'>
            ...
            <Select
              isMulti
              options={UsersList.filter((user) =>
                assignedMembersList.includes(user._id)
              ).map((user) => ({label: user.name, value: user._id}))}
              value={assignedMembers}
              onChange={(e) => setAssignedMembers(e)}
              onBlur={() => validateMembers(assignedMembers)}
              onFocus={() => setMembersError("")}
              isClearable={false}
            />
